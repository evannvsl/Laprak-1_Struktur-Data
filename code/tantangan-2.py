...
- Bubble Sort    : O(n²) perbandingan & pertukaran pada average/worst case.
  Best case (data sudah terurut) masih O(n²) jika tidak diberi flag optimasi.
- Selection Sort : O(n²) perbandingan di semua kasus, pertukaran O(n).
  Tidak dipengaruhi urutan data.
- Insertion Sort : O(n²) perbandingan/shift di average/worst case,
  tetapi O(n) pada data menaik (best case) karena hanya 1 perbandingan per elemen.

HASIL PENGAMATAN (lihat grafik):
- Pada data menaik, Insertion Sort sangat cepat (hampir tidak ada shift).
- Bubble dan Selection Sort tetap lambat meskipun data terurut.
- Pada data menurun (worst case), semua algoritma menunjukkan performa terburuk.
- Grafik waktu terhadap n pada data acak menunjukkan kurva kuadratik (O(n²)).
""")